...
- **Transport strategies**:
  ...
  - WebSocket transport can pass `?secret=<hex>` in the URL. When `Router.handleNewConnection` receives an existing secret, it reuses the same `ServerConnection` object and replaces only the transport.
...
- The server trusts possession of the connection `secret` as proof of connection ownership. There is no additional binding to network identity or HTTP cookie/session.
...
- Bind the `secret` more tightly to transport establishment, optionally with a server-issued nonce or signed reconnect token.
